...
## Corpus Check
- 70 files · ~35,871 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 758 nodes · 1753 edges · 51 communities detected
- Extraction: 65% EXTRACTED · 35% INFERRED · 0% AMBIGUOUS · INFERRED: 608 edges (avg confidence: 0.5)
...
4. `DetectionPolicy` - 38 edges
5. `SupervisorStepResult` - 37 edges
6. `YoloPrompt` - 35 edges
7. `NullTurretController` - 35 edges
8. `EventRecordingConfig` - 33 edges
9. `CounterZone` - 33 edges
10. `YoloRuntimeConfig` - 32 edges
...
Cohesion: 0.1
Nodes (60): BenchConfig, CalibrationConfig, UiButton, EventRecordingConfig, ServoLimits, YoloPrompt, NullTurretController, ControllerSession (+52 more)

### Community 1 - "Community 1"
Cohesion: 0.06
Nodes (50): AlgorithmReplayRow, AlgorithmReplaySummary, _denormalize_yolo_bbox(), discover_replay_samples(), _is_augmented_or_repeated(), pair_fixed_with_nearest_turret(), PairedReplaySample, parse_sample_stem() (+42 more)

### Community 2 - "Community 2"
Cohesion: 0.07
Nodes (26): Enum, _append_form_field(), _build_discord_file_request(), build_event_video_recorder(), DiscordWebhookPublisher, EventVideoFinalize, NoopEventVideoPublisher, TurretEventRecorder (+18 more)
...
Cohesion: 0.1
Nodes (10): build_request(), ControllerRequest, ControllerResponse, open(), Detach servos to stop buzzing. They re-engage on next movement., Set continuous servo velocity in degrees/sec. Pico interpolates at 50Hz., Map camera-POV deltas to physical servo-angle deltas., RP2040SerialController (+2 more)

### Community 6 - "Community 6"
Cohesion: 0.13
Nodes (27): build_bootstrap_command(), build_deploy_steps(), build_jetson_gpu_setup_command(), build_rsync_command(), build_ssh_command(), build_udev_install_command(), deploy(), DeployStep (+19 more)

### Community 7 - "Community 7"
Cohesion: 0.14
Nodes (8): Controller, _error(), main(), _ok(), parse_args(), Stop PWM signal to eliminate servo buzz., run_app(), Servo

### Community 8 - "Community 8"
Cohesion: 0.13
Nodes (19): deploy_files(), _enter_raw_repl(), _exec_raw(), _hard_reset(), MicroPythonDeployError, Raised when firmware deployment to the Pico fails., _read_until(), _reset_input_buffer() (+11 more)
...
Cohesion: 0.2
Nodes (7): DeepStreamPerceptionAdapter, Placeholder for the fixed-camera DeepStream pipeline integration., EventVideoPublisher, PerceptionAdapter, Return the latest frame metadata and detections., PerceptionAdapter, Protocol
...
Cohesion: 0.39
Nodes (5): test_main_parses_cat_dataset_collection_options(), test_main_uses_event_recording_config(), test_main_uses_yolo_image_size_from_app_config(), test_main_yolo_image_size_cli_overrides_app_config(), _write_app_config()
...
Cohesion: 0.48
Nodes (5): _draw_circular_button(), _draw_eye(), _lerp(), _require_cv2(), run_eye_screen()
...
- **Thin community `Community 43`** (2 nodes): `main()`, `prepare_cat_finetune_dataset.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 44`** (2 nodes): `tag_zone_frame_size.py`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 45`** (2 nodes): `test_rebuild_yolo_engine_script_exports_configurable_tensor_rt_engine()`, `test_rebuild_yolo_engine_script.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 46`** (2 nodes): `test_x11_tracking_runner_checks_display_and_launches_tracking_ui()`, `test_tracking_x11_script.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 47`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 48`** (1 nodes): `patch_torchvision_jetson.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 49`** (1 nodes): `pico_config.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `RP2040ProtocolError` connect `Community 0` to `Community 9`, `Community 5`, `Community 6`, `Community 15`?**
  _High betweenness centrality (0.073) - this node is a cross-community bridge._
- **Why does `Detection` connect `Community 0` to `Community 1`, `Community 2`, `Community 4`, `Community 14`, `Community 20`, `Community 23`, `Community 30`?**
  _High betweenness centrality (0.068) - this node is a cross-community bridge._
- **Why does `SupervisorStepResult` connect `Community 0` to `Community 1`, `Community 2`, `Community 20`?**
...
- **Are the 37 inferred relationships involving `DetectionPolicy` (e.g. with `YoloPrompt` and `VisionConfig`) actually correct?**
  _`DetectionPolicy` has 37 INFERRED edges - model-reasoned connections that need verification._