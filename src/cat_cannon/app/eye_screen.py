...
def run_eye_screen(config: EyeConfig) -> ScreenName | None:
    ...
    def _detection_loop():
        ...
        while _detect_running:
            ...
            if (
                ...
                from cat_cannon.app.heartbeat import Liveness
                _hb["cfg"] = _hb_cfg
            ...
            if _ctrl is not None and _sys_config is not None:
                idle_centering.update(
                    controller=_ctrl,
                    armed=state.armed,
                    detection_present=has_centering_activity(
                        human_present=step_result.human_present,
                        correction_present=step_result.correction is not None,
                    ),
    ...
    def _health_monitor():
        """Watch loop liveness and signal the guardian when the app hangs."""
        ...
        while _detect_running and not _fault_event.is_set():
            ...
            if not state.armed:
                ...
                # offline, so suppress heartbeat-driven restarts.
                continue
            _live = _hb["live"]
            reason = "hang" if _live is not None and _live.is_stale() else None
